<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/caiospessoa/caiospessoa/main/header-dark.png">
  <img src="https://raw.githubusercontent.com/caiospessoa/caiospessoa/main/header-light.png" width="100%"/>
</picture>

<div align="center">

[![LinkedIn](https://img.shields.io/badge/-LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/caiopessoa)
&nbsp;
[![Email](https://img.shields.io/badge/-Email-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:caiospessoa@gmail.com)
&nbsp;
[![Zaandam](https://img.shields.io/badge/🏍️_Zaandam-NL-009448?style=for-the-badge)](https://en.wikipedia.org/wiki/Zaandam)

</div>

---

```
Brazilian living in Zaandam. Riding motorcycles through the city and trying to be a better BOXER!
Building observability platforms by day, breaking things to understand them.

Network engineer turned SRE — from HFT trading floors and Arista switches
to cloud-native stacks on Kubernetes. I've always cared about the signal
underneath the noise.
```

---

## 🧰 What I work with

**Observability**
&nbsp;
![Grafana](https://img.shields.io/badge/Grafana-F46800?style=flat-square&logo=grafana&logoColor=white)
![Prometheus](https://img.shields.io/badge/Prometheus-E6522C?style=flat-square&logo=prometheus&logoColor=white)
![Loki](https://img.shields.io/badge/Loki-F15A29?style=flat-square&logo=grafana&logoColor=white)
![Tempo](https://img.shields.io/badge/Tempo-F46800?style=flat-square&logo=grafana&logoColor=white)
![Mimir](https://img.shields.io/badge/Mimir-009448?style=flat-square&logo=grafana&logoColor=white)
![OpenTelemetry](https://img.shields.io/badge/OTel-425CC7?style=flat-square&logo=opentelemetry&logoColor=white)

**Cloud & Infra**
&nbsp;
![Azure](https://img.shields.io/badge/Azure-0078D4?style=flat-square&logo=microsoftazure&logoColor=white)
![Kubernetes](https://img.shields.io/badge/Kubernetes-326CE5?style=flat-square&logo=kubernetes&logoColor=white)
![Terraform](https://img.shields.io/badge/Terraform-7B42BC?style=flat-square&logo=terraform&logoColor=white)
![Ansible](https://img.shields.io/badge/Ansible-EE0000?style=flat-square&logo=ansible&logoColor=white)

**Query languages I actually enjoy**
&nbsp;
![PromQL](https://img.shields.io/badge/PromQL-E6522C?style=flat-square&logo=prometheus&logoColor=white)
![LogQL](https://img.shields.io/badge/LogQL-F15A29?style=flat-square&logo=grafana&logoColor=white)
![KQL](https://img.shields.io/badge/KQL-0078D4?style=flat-square&logo=microsoftazure&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)

**Networking roots**
&nbsp;
![Arista](https://img.shields.io/badge/Arista-FF6600?style=flat-square)
![BGP](https://img.shields.io/badge/BGP-005073?style=flat-square)
![gNMI](https://img.shields.io/badge/gNMIc-4285F4?style=flat-square)
![SNMP](https://img.shields.io/badge/SNMP-6C757D?style=flat-square)

---

## 🛤️ How I got here

| Period | Where | What I was doing |
|--------|-------|-----------------|
| Early career | Brazil → EU | Network engineering, DC infra, cross-connects |
| HFT era | Zaandam | Low-latency networks, Arista/Metamako, gNMI telemetry, BGP |
| Streaming & retail | Netherlands | Shifting from pure networking to infra observability |
| Now | Zaandam | SRE / DevOps, cloud-native, LGTM stack on AKS |

---

## 🔭 Things I've built or contributed to

- **Prometheus exporter** — GitHub Secret Scanning metrics → Mimir → Grafana OSS
- **AKS observability dashboards** — multi-region, multi-datasource (AMP + LAW), production-grade
- **gNMI-based network telemetry** — Arista + Cisco NXOS, BGP/LACP/interfaces/PSU monitoring
- **LGTM platform self-monitoring** — dogfooding the stack to watch the stack
- **NetBox automation** — IP scanning with Python + Airflow DAGs

---

## 🔄 Currently

```yaml
learning:
  - Boxing  # new challenge outside the terminal
  - Distributed tracing patterns with OpenTelemetry
  - Technical leadership & platform strategy

working_on:
  - LGTM observability platform on AKS
  - Grafana dashboard design for cloud-native workloads

not_doing:
  - Pushing to this repo nearly enough
```

---

## 🏍️ Outside the terminal

Zaandam commuter on a naked bike — A10, A8, A7, whatever the ring roads throw at me.
Training 5x a week (push / pull / legs), obsessing over macros.
Brazilian abroad, always looking for decent coffee, a good road, and a Saturday with football on — grew up supporting a green São Paulo club that plays at a ground surrounded by palm trees. You probably know the one.

---

<div align="center">

*"The map is not the territory — but a good dashboard gets pretty close."*

<br/>

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:009448,100:0f0f0f&height=100&section=footer" width="100%"/>

</div>
